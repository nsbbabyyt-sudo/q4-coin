...
export default function Page() {
  ...
  useEffect(() => {
    ...
    fetch("/api/public/payout-meta")
      ...
      .catch(() =>
        !cancelled && setMeta({ ok: false, lastRunAt: null, periodDays: 7 })
      );
    return () => {
      cancelled = true;
    };
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6 px-6">
          <div className="glow-card p-6 rounded-2xl border border-white/10 bg-white/5">
            ...
            <p className="mt-2 text-sm text-white/70">
              Launch: Oct 5 (6:00 PM CT)
            </p>
          </div>

          <div
            id="stake"
            className="glow-card p-6 rounded-2xl border border-white/10 bg-white/5"
          >
            <StakePanel />
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 pb-10">
          ...
          <div className="glow-card p-4 rounded-2xl border border-white/10 bg-white/5 space-y-3">
            <div className="inline-flex items-center gap-2 rounded-full border border-[var(--accent,#0A84FF)]/40 bg-[var(--accent,#0A84FF)]/15 px-3 py-1 text-[var(--accent,#0A84FF)] text-xs font-semibold">
              Tier 1: ≥100,000 share 30% fees (pro-rata) · Tier 2: ≥1,000,000
              get weekly top-meme airdrop
            </div>

            <h3 className="font-semibold">Tokenomics</h3>
            <ul className="text-sm text-white/80 space-y-2">
              <li>
                <span className="font-semibold text-[var(--accent,#0A84FF)]">
                  Creator Fee Share —
                </span>{" "}
                30% of creator fees split weekly among wallets with ≥ 100,000
                staked (pro-rata).
              </li>
              <li>
                <span className="font-semibold">Special Airdrop —</span> #1
                trending Solana memecoin split equally among wallets with ≥
                1,000,000 staked.
              </li>
              <li>
                <span className="font-semibold">Distribution Timing —</span>{" "}
                Randomized once per week to reduce gaming. See “Last
                distribution” below.
              </li>
              <li className="text-white/70">
                <span className="font-semibold">Staking —</span> Soft-stake (no
                lock). Sign once here.
              </li>
            </ul>
            ...
            <div className="mt-4">
              <div className="h-2 w-full rounded-full bg-white/10 overflow-hidden border border-white/10">
                <div
                  className="h-full bg-[var(--accent,#0A84FF)]"
                  style={{ width: "30%" }}
                />
              </div>
          ...
          {/* MARKET / CHART */}
          <div className="glow-card p-4 rounded-2xl border border
