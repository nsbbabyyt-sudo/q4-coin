      ...
      <main className="min-h-screen bg-gradient-to-b from-black to-zinc-900 text-white space-y-10">
        ...
        <header className="flex items-start justify-end pr-6 pt-4">
          <div className="mr-3 mt-2">
            {/* Slight offset + 1.5x size */}
            <div className="translate-x-[-0.25rem] translate-y-[0.25rem] scale-[1.5] origin-top-right inline-block">
              <WalletConnect />
            </div>
          </div>
        </header>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 gap-6 px-6 pb-10">
          ...
          <div className="glow-card p-4 rounded-2xl border border-white/10 bg-white/5 space-y-3">
            ...
            <ul className="text-sm text-white/80 space-y-2">
              <li><span className="font-semibold text-[var(--accent,#0A84FF)]">Creator Fee Share —</span> 30% of creator fees split weekly among wallets with ≥ 100,000 staked (pro-rata).</li>
              <li><span className="font-semibold">Special Airdrop —</span> #1 trending Solana memecoin split equally among wallets with ≥ 1,000,000 staked.</li>
              <li><span className="font-semibold">Distribution Timing —</span> Randomized once per week to reduce gaming. See “Last distribution” below.</li>
              <li className="text-white/70"><span className="font-semibold">Staking —</span> Soft-stake (no lock). Sign once here.</li>
            </ul>

            {/* Status pill */}
            <div className="mt-3 rounded-xl border border-white/10 bg-white/5 p-3">
              <div className="text-xs text-white/60">Last distribution</div>
              <div className="text-sm font-medium">{meta ? formatWhen(meta.lastRunAt) : "Loading…"}</div>
              <div className="mt-1 text-xs text-white/60">{meta ? nextWindowHint(meta.lastRunAt, meta.periodDays) : ""}</div>
            </div>

            {/* Progress bar (static example) */}
            <div className="mt-4">
              ...
              <div className="mt-3">
                <a href="#stake" className="text-sm text-[var(--accent,#0A84FF)] hover:underline">Register now ↗</a>
              </div>
            </div>
          </div>
          ...
          <div className="glow-card p-4 rounded-2xl border border-white/10 bg-white/5">
            ...
            <div className="relative w-full overflow-hidden rounded-xl border border-white/10" style={{ paddingTop: "56.25%" }}>
              <iframe src={dexs} title="Dex chart" className="absolute inset-0 w-full h-full" style={{ border: 0 }} loading="lazy" allow="clipboard-write; fullscreen" />
            </div>
            <div className="flex justify-end mt-2">
              <a href={dexs} target="_blank" className="text-sm text-[var(--accent,#0A84FF)] hover:underline" rel="noreferrer">Open chart ↗</a>
            </div>
          </div>
        </section>
